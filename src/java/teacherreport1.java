 ...
 public class teacherreport1 extends javax.servlet.http.HttpServlet implements javax.servlet.Servlet {
   static final long serialVersionUID = 1L;
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		System.out.println("finalyy yeyyyyyy "+subject);
		String teachername=request.getParameter("tname");
		String[] subs=subject.split(",");
		String dname=subs[0];
		String year=subs[1];
		String sem=subs[2];
		String section=subs[3];
		String sub=subs[4];
		
		System.out.println("D: "+dname+" T: "+teachername+" S: "+sub);
		double i=0;
		...
		String prfname="";
		try{
			String str,str1,str2,sql,yearq,semq,sectionq;
			int m,n;
			double x,y,z,v;
			for(m=0;m<8;m++)
			{
			...
			sql="select * from teacher where uname='"+teachername+"'";
			rs=s.executeQuery(sql);
			
			String tname="";
			while(rs.next()){
				tname=rs.getString("sno");
				prfname=rs.getString("name");
			}
			...
			while(rs.next())
			{
				...
				yearq=rs.getString("year");
				semq=rs.getString("sem");
				sectionq=rs.getString("section");
				System.out.println("from web "+dname+" "+tname+" "+sub+" "+year+" "+sem+" "+section);
				System.out.println("from db "+str+" "+str1+" "+str2+" "+yearq+" "+semq+" "+sectionq);
				System.out.println(dname.compareTo(str) +"&&"+ tname.compareTo(str1) +"&&"+ sub.compareTo(str2)
						+"&&"+year.equals(yearq)+"&&"+sem.equals(semq)+"&&"+section.equals(sectionq));
		...
		out.println("<center>" +
					...
					"</tr>" +
					"<tr>" +
					"<th align=left>Maintains Discipline of Class :</th>" +
		...
		{
			{out.println("NO FEEDBACK SUBMITTED ");}
		}
	}   	  	    
}